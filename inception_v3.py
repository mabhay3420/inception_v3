...
def get_image():
    ...
    Downloaded from here: https://github.com/EliSchwartz/imagenet-sample-images
    """
    image_dir = Path("images")
    # all files in the directory
    images = list(image_dir.glob("*"))
    # return a random image from the directory
    rnd = torch.randint(0, len(images), (1,)).item()
    return images[rnd]


def print_result(result):
    """
    Prints the result of the model inference.
    """
    print("Original result shape", result[0].shape)
    print("Applying softmax to the result")
    probabilities = F.softmax(result[0], dim=0)
    print("Probabilities shape", probabilities.shape)
    top5_prob, top5_catid = torch.topk(probabilities, 5)
    separator()
    print("Top 5 categories and probabilities:")
    separator()
    for i in range(top5_prob.size(0)):
        print(_IMAGENET_CATEGORIES[top5_catid[i]], top5_prob[i].item())
    separator()
...
if __name__ == "__main__":
    # Get weights
    weights = Inception_V3_Weights.verify(Inception_V3_Weights.IMAGENET1K_V1)
    # get model
    model = Inception3(num_classes=len(weights.meta["categories"]))
    ...
    print("Visualizing model using model explorer")
    separator()
    ep = torch.export.export(model, (input_batch,))
    model_explorer.visualize_pytorch("Inception V3", exported_program=ep)
